...
import { Chat, useAppContext } from '@/context/AppContext'
...
const ChatLabel = ({ openMenu, setOpenMenu, id, name }: { openMenu: OpenMenu, setOpenMenu: (openMenu: OpenMenu) => void, id: string, name: string }) => {
    ...
    const selectChat = () => {
        ...
        setSelectedChat(chatData as Chat)
        ...
        <>
            <div onClick={selectChat} className='flex items-center justify-between p-2 text-white/80 hover:bg-white/10 rounded-lg text-sm group cursor-pointer'>
                ...
                <div onClick={e => { e.stopPropagation(); setOpenMenu({ id: id, open: !openMenu.open }) }}
                    ...
                    <Image src={assets.three_dots} alt='three_dots' className={`w-4 ${openMenu.id === id && openMenu.open ? '' : 'hidden'} group-hover:block`} />
